...
REMOTE_URI = f"mongodb://{DATABASE_USERNAME}:{DATABASE_PASSWORD}@{DATABASE_SERVER}:{DATABASE_PORT}/{DATABASE_NAME}"
LOCAL_URI = REMOTE_URI.replace(DATABASE_SERVER, "localhost")
...
def init(ex=None):
    ...
    if '--unobserved' in sys.argv:
        return ex
    if ('-F' in sys.argv) or ('--file_storage' in sys.argv):
        return ex
    if test_connection(REMOTE_URI):
        ex.observers.append(MongoObserver.create(REMOTE_URI, db_name=DATABASE_NAME))
    else:
        ssh_uri = open_ssh()
...
def open_ssh():
    ...
    ssh_string = f"ssh -N -L {open_port}:{DATABASE_MACHINE}:{DATABASE_PORT} {REMOTE_SERVER_USERNAME}@{REMOTE_SERVER}"
    p = subprocess.Popen([ssh_string], shell=True)
    print(ssh_string)
